...
En primera instancia, mi idea inicial era dividir el espacio de búsqueda en sectores, de forma que se forzara a combatir a dos soluciones cuando se encontraban en el mismo. Sin embargo, esto resulta difícil de implementar, pues es costoso (escala muy mal con las dimensiones), y podría generar absurdos (dos soluciones muy cercanas que no combatan).

Por tanto, haremos lo siguiente: **los jugadores estarán representados como soluciones**, unos vectores de componentes reales. Tantas como dicte nuestra dimensión. Simularemos los anillos a la inversa: de cada **solución partirá una bola n-dimensional**, que irá aumentando su radio conforme pasen las generaciones. Al principio, éste será pequeño. Con el tiempo, crecerá, lo que hará que las bolas interseque con las de otras soluciones. Cuando esto ocurra, las haremos combatir. La peor de las dos morirá, y la otra seguirá avanzando.
...
Intentaremos seguir este hilo de pensamiento, de forma que atajemos nuestros problemas uno a uno.

### Versión 2: sondeando el terreno

Primero, desarrollemos la última idea de antes. Para no empezar con soluciones que no nos garantizan nada, modificaremos la generación inicial para quedarnos con las mejores. Eso debería reducir el número de evaluaciones necesarias hasta llegar a un mínimo local.
        ...
        if (t % periodo_generacional == 0) {
            if (poblacion.size() == 1) {
                for (int i = 0; i < poblacion_inicial; i++) {
                    poblacion.push(mutar(poblacion[0]))
                }
            }
            else {
                Comprobar si en `poblacion` existen dos o más elementos con distancia entre ellos menor que radio.
...
### Versión 4: controlando el radio
...
Lo que conseguimos con esto es que, durante las primeras generaciones, el radio crece lo suficientemente lento como para que la búsqueda local sea efectiva, y las soluciones se muevan hacia óptimos locales de manera más o menos rápida. Cuando el radio sea considerablemente grande, empezarán a luchar. Finalmente, cuando solo quede una, se irá repoblando, haciendo que luche contra hijos de la mejor.

Para hacer más justa la última fase, reduciremos el radio para que les dé tiempo a las mutaciones a desarrollarse y no mueran en cuanto nazcan. Actualmente, se ha usado `radio = radio/5`. Produce un buen equilibrio entre desarrollo y malgasto de evaluaciones del fitness.
...
De nuevo, podemos observar que al principio del algoritmo se consiguen muy buenos resultados. Esto es debido al esquema de generación de vecinos. No obstante, a partir del 10% de evaluaciones, se pierde la ventaja que se había obtenido. Es de esperar, puesto que la fase de mejora de soluciones en la búsqueda local "desperdicia" varias evaluaciones. En el tramo final, se recupera cierta ventaja (especialmente ante PSO) debido a la nueva progresión implementada.

No obstante, se necesita mejorar el rendimiento claramente, pues la mayor parte de las veces funciona mejor Differential Evolution.
...
Tras esto, nos hemos dado cuenta de que necesitamos mejorarla. Para ello, hemos razonado en cómo se comporta, y consecuentemente,  realizamos una mejora en las fases iniciales. Para afinar el crecimiento, y siguiendo el modelo de enfriamiento simulado, hemos implementado un esquema distinto de crecimiento del radio. Finalmente, hemos tomado una mutación de los algoritmos genéticos para añadir diversificación + explotación en las últimas fases.

Quizás, el mayor problema de metaheurística battle royale es la inversión del esquema de diversificación-explotación con respecto a los algoritmos habituales, y eso es lo que está generando los problemas de las últimas generaciones. Sería necesario un análisis más exhaustivo para demostrarlo.

#### Posibles mejoras

Intentemos razonar sobre cuáles son las áreas en las que podríamos seguir mejorándolo.

Uno de los principales problemas que tenemos es el **gasto de evaluaciones que producen las primeras fases**. Se intenta ver cuál es la mejor parte del terreno, por lo que partimos de soluciones mediocres, y poco a poco las mejoramos hasta que se vuelven decentes. Hacer una búsqueda local es caro, y deberíamos intentar aplicarla las mínimas veces posibles. Y, si lo hacemos, necesitamos que se consiga una disminución del fitness importante. Es por ello que debemos **ajustar lo más finamente posible nuestra búsqueda local**. Podríamos intentar analizar cuáles son las soluciones que mayor disminución del fitness consiguen en una sola BL, de forma que se prioricen en la exploración.

Otra de las formas las que se puede seguir mejorando el algoritmo es mediante un **cambio en el esquema de aumento del radio**. Podríamos intentar hacerlo dinámico, de forma que aumente en mayor medida conforme menos evaluaciones resten del total. De esta forma, se vuelve más agresivo en las últimas partes, priorizando las mejores soluciones hasta el momento.

En lo que respecta a la última parte del algoritmo, claramente concluimos que **nos quedamos estancados en óptimos locales**, y no podamos salir de ellos. Lo intentamos arreglar con nuestra mutación, pero no ha conseguido su propósito de forma satisfactoria. Es posible que se deba a que una única solución no proporciona suficiente diversidad. Para solucionarlo, podríamos intentar cruzar varias soluciones distintas. No obstante, esto haría que nuestro algoritmo se parecía mucho a un memético, por lo que perdería interés el ejercicio.

Parece evidente por tanto, que la **inspiración natural hace que converjamos en clases de algoritmos similares**. Siguiendo estas últimas ideas, llegaríamos a un memético como hemos comentado, aún siendo la idea original bastante diferente. ¿Quizás estemos usando metaconocimiento del medio, y eso hace que acaben siendo iguales? Llegados a este punto, no sería una mala idea volver a nuestra inspiración, con el fin de abstraer nuevos conceptos e implementarlos.
